...
def kernel(
    ...
    mma_tiler_mnk = (256, 256, tile_k)
    threads_per_cta = 32 * (epi_warps + 2)
    epilogue_warp_ids = tuple(range(epi_warps))
    ...
    ab_mbar_ptr = smem.allocate_array(cutlass.Int64, ab_stages * 2, byte_alignment=8)
    acc_mbar_ptr = smem.allocate_array(cutlass.Int64, acc_stages * 2, byte_alignment=8)
    tmem_dealloc_mbar = smem.allocate(cutlass.Int64, byte_alignment=8)
    ...
    ab_producer, ab_consumer = pipeline.PipelineTmaUmma.create(
        barrier_storage=ab_mbar_ptr.align(min_align=8),
    ...
    acc_producer, acc_consumer = pipeline.PipelineUmmaAsync.create(
        barrier_storage=acc_mbar_ptr.align(min_align=8),
        num_stages=acc_stages,
        producer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread),
        consumer_group=pipeline.CooperativeGroup(
            pipeline.Agent.Thread,
            cute.size(cta_layout_vmnk, mode=[0]) * epi_warps,
        ),
        cta_layout_vmnk=cta_layout_vmnk,
    ).make_participants()
...
def host_function(
    ...
    b: cute.Tensor,
    c: cute.Tensor,
    tile_k: cutlass.Constexpr = 64,
    ab_stages: cutlass.Constexpr = 4,
    epi_warps: cutlass.Constexpr = 4,
):
    mma_tiler_mnk = (256, 256, tile_k)
    threads_per_cta = 32 * (epi_warps + 2)
...
def validate_mnk(mnk: Tuple[int, int, int], tile_k: int = 64) -> None:
    m, n, k = mnk
    if m % 256 != 0 or n % 256 != 0 or k % tile_k != 0:
        raise ValueError(f"mnk must be divisible by (256, 256, {tile_k}), got {mnk}")
...
def run_dense_gemm(a, b, *, tile_k: int = 64, ab_stages: int = 4, epi_warps: int = 4):
    validate_mnk((a.shape[0], b.shape[0], a.shape[1]), tile_k=tile_k)
    c, a_tensor, b_tensor, c_tensor = prepare_cute_gemm(a, b)
    return run_dense_gemm_prepared(
        c,
        a_tensor,
        b_tensor,
        c_tensor,
...
def _compile_candidate(a_tensor, b_tensor, c_tensor, tile_k=64, ab_stages=4, epi_warps=4):
    validate_mnk(
        (a_tensor.shape[0], b_tensor.shape[0], a_tensor.shape[1]),
        tile_k=tile_k,
    )
    compiled = cute.compile(
        host_function,
    ...
    return lambda: compiled(a_tensor, b_tensor, c_tensor)
...
def autotune_config(
    ...
    params_dict = params_dict or DEFAULT_AUTOTUNE_SPACE
    return cute_testing.tune(
        _compile_candidate,
        params_dict=params_dict,
        kernel_arguments=cute_testing.JitArguments(a_tensor, b_tensor, c_tensor),
        warmup_iterations=warmup,
        iterations=iters,
    )


def run_dense_gemm_autotuned(a, b, params_dict=None, warmup: int = 5, iters: int = 20):
    c, a_tensor, b_tensor, c_tensor = prepare_cute_gemm(a, b)
    best = autotune_config(
        a_tensor,
        b_tensor,
        c_tensor,
        params_dict=params_dict,
    ...
    compiled = cute.compile(host_function, a_tensor, b_tensor, c_tensor, **best)
    compiled(a_tensor, b_tensor, c_tensor)
    return c, best
...
def main() -> None:
    ...
    parser.add_argument("--mnk", type=str, default="1024,1024,256")
    parser.add_argument("--tile-k", type=int, default=64)
    parser.add_argument("--ab-stages", type=int, default=4)
    parser.add_argument("--epi-warps", type=int, default=4)
    ...
    mnk = tuple(int(x.strip()) for x in args.mnk.split(","))
    a, b = make_inputs(mnk)
    if args.autotune:
        got, best = run_dense_gemm_autotuned(
            a,
            b,
            warmup=args.warmup,
            iters=args.iters,
        )
        print("AUTOTUNE_BEST", best)
    else:
        got = run_dense_gemm(
            a,
            b,
    ...
    print("PASS", {"mnk": mnk, "variant": "2cta_tma_autotune_singlekernel", "config": best})

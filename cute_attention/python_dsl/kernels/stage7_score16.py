...
MAX_SEQ_LEN_FOR_STAGE7_CUTE = 4096
_STAGE7_COMPILED_CACHE = {}


if HAS_CUTE:
    def _make_stage7_host(seq_len: int, head_dim: int, block_m: int, block_n: int, num_threads: int):
        @cute.kernel
        def stage7_q16_kv16_score16_kernel(
            q: cute.Tensor,
            k: cute.Tensor,
            v: cute.Tensor,
            o: cute.Tensor,
            ...
            score_ptr = smem.allocate_array(cutlass.Float16, num_elems=block_m * block_n)
            ...
            for n_start in range(0, seq_len, block_n):
                ...
                for lin in range(tidx, block_m * block_n, num_threads):
                    ...
                    score_tile[lin] = score.to(cutlass.Float16)
                cute.arch.barrier()
                ...
                for row in range(tidx, block_m, num_threads):
                    ...
                    if q_idx < seq_len and n_start <= q_idx:
                        ...
                        for blk_j in range(block_n):
                            s = score_tile[row_base + blk_j].to(cutlass.Float32)
                        ...
                        for blk_j in range(block_n):
                            ...
                            if kv_idx < seq_len and kv_idx <= q_idx:
                                p = cute.math.exp(score_tile[row_base + blk_j].to(cutlass.Float32) - m_new)
                            score_tile[row_base + blk_j] = p.to(cutlass.Float16)
                            block_sum += p
                    ...
                    if q_idx < seq_len and n_start <= q_idx:
                        ...
                        for blk_j in range(block_n):
                            ...
                            if kv_idx < seq_len and kv_idx <= q_idx:
                                ...
                                acc += score_tile[row_base + blk_j].to(cutlass.Float32) * v_tile[v_base + d_idx].to(cutlass.Float32)
                        acc_tile[lin] = acc
                cute.arch.barrier()
        ...
        def stage7_forward_host(
            ...
            stage7_q16_kv16_score16_kernel(
                q,
                k,
                v,
                o,
        ...
        return stage7_forward_host


def stage7_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage7 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage7 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage7 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE7_CUTE:
        raise ValueError(
            f"stage7 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE7_CUTE}, got {seq_len}."
        )
    if head_dim > 256:
        raise ValueError(f"stage7 currently supports head_dim <= 256, got {head_dim}.")
    if config.block_m <= 0:
    ...
    smem_bytes = (
        2 * (config.block_m * head_dim + config.block_m * config.block_n + 2 * config.block_n * head_dim)
        + 4 * (config.block_m * head_dim + 2 * config.block_m)
    )
    if smem_bytes > 96 * 1024:
        raise ValueError(
            f"stage7 shared memory footprint too large ({smem_bytes} bytes). "
...
def _stage7_compile(cache_key, q_cute, k_cute, v_cute, o_cute, softmax_scale, block_m, block_n, num_threads):
    compiled = _STAGE7_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        seq_len = cache_key[0][1]
        head_dim = cache_key[0][2]
        stage7_host = _make_stage7_host(
            seq_len=seq_len,
            head_dim=head_dim,
            block_m=block_m,
            block_n=block_n,
        ...
        _STAGE7_COMPILED_CACHE[cache_key] = compiled
    return compiled

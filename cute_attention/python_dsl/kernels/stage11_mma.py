...
MAX_SEQ_LEN_FOR_STAGE11_CUTE = 4096
_STAGE11_COMPILED_CACHE = {}


if HAS_CUTE:
    class Stage11FlashAttentionAmpere:
        ...
        def kernel(
            ...
            tKVpKV = cute.make_rmem_tensor(
                cute.make_layout((tKsK.shape[0][1], cute.size(tKsK, mode=[1]), cute.size(tKsK, mode=[2])), stride=(cute.size(tKsK, mode=[2]), 0, 1)),
                cutlass.Boolean,
            )
            ...
            mma_params = SimpleNamespace(thr_mma=thr_mma, tiled_mma=tiled_mma, tSrQ=tSrQ, tSrK=tSrK, tOrVt=tOrVt, acc_O=acc_O)
...
def stage11_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage11 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage11 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage11 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE11_CUTE:
        raise ValueError(
            f"stage11 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE11_CUTE}, got {seq_len}."
        )
    if not Stage11FlashAttentionAmpere.can_implement(cutlass.Float16, head_dim, config.block_m, config.block_n, config.num_threads, True):
        raise ValueError("stage11 config is not supported by the MMA kernel constraints.")
...
def _stage11_compile(cache_key, q_cute, k_cute, v_cute, o_cute, scale, current_stream, head_dim, block_m, block_n, num_threads):
    compiled = _STAGE11_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        kernel = Stage11FlashAttentionAmpere(
            head_dim=head_dim,
        ...
        _STAGE11_COMPILED_CACHE[cache_key] = compiled
    return compiled

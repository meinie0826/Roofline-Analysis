...
from .stage17_warpspec_backend import Stage17FlashAttentionWarpSpecMultistage
...
def _stage17_can_implement(head_dim: int, config: AttentionConfig) -> bool:
    return Stage17FlashAttentionWarpSpecMultistage.can_implement(
        cutlass.Float16,
        head_dim,
        config.block_m,
        config.block_n,
    ...
    for num_stages_kv in stage_values:
        for block_m in block_m_values:
            for block_n in block_n_values:
                ...
                if not _stage17_can_implement(head_dim, tuned):
                    continue
                try:
...
def _stage17_forward_impl(q, k, v, config: AttentionConfig):
    ...
    if seq_len > MAX_SEQ_LEN_FOR_STAGE17_CUTE:
        raise ValueError(f"stage17 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE17_CUTE}, got {seq_len}.")
    if config.num_stages_kv not in {0, 2, 3}:
        raise ValueError(f"stage17 currently supports num_stages_kv in {{2, 3}}, got {config.num_stages_kv}.")

    normalized = replace(config, num_threads=256, num_stages_kv=(config.num_stages_kv or 3), autotune=False)
    if not _stage17_can_implement(head_dim, normalized):
        raise ValueError("stage17 config is not supported by the warpspec multistage kernel constraints.")
...
def stage17_forward(q, k, v, config: AttentionConfig | None = None):
    ...
    return _stage17_forward_impl(q, k, v, replace(tuned, autotune=False, num_threads=256))
...
def _stage17_compile(cache_key, q_cute, k_cute, v_cute, o_cute, scale, current_stream, head_dim, block_m, block_n, num_threads, num_stages_kv):
    ...
    if compiled is None:
        kernel = Stage17FlashAttentionWarpSpecMultistage(
            head_dim=head_dim,

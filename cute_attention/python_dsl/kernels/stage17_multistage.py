...
MAX_SEQ_LEN_FOR_STAGE17_CUTE = 4096
_STAGE17_COMPILED_CACHE = {}
_STAGE17_AUTOTUNE_CACHE = {}
_STAGE17_AUTOTUNE_CACHE_PATH = Path(__file__).resolve().parents[1] / ".cache" / "stage17_autotune.json"


if HAS_CUTE:
    class Stage17FlashAttentionAmpere:
...
def _make_stage17_config(config: AttentionConfig, *, block_m: int, block_n: int, num_stages_kv: int) -> AttentionConfig:
...
def _stage17_candidate_values(preferred: int, values: list[int], *, limit: int) -> list[int]:
    ordered = []
...
def autotune_stage17_config(
    ...
    *,
    ...
    if cached_disk is not None:
        ...
        if "num_threads" in cached_disk:
            tuned = replace(tuned, num_threads=int(cached_disk["num_threads"]))
        _STAGE17_AUTOTUNE_CACHE[cache_key] = tuned
        return tuned

    block_m_values = _stage17_candidate_values(config.block_m, [128, 96, 64, 48, 32, 16], limit=seq_len)
    block_n_values = _stage17_candidate_values(config.block_n, [256, 192, 128, 96, 64], limit=seq_len)
    stage_values = _stage17_candidate_values(config.num_stages_kv or 3, [5, 4, 3, 2], limit=5)
    thread_values = _stage17_thread_values(config.num_threads)
    ...
    if best_config is None:
        raise ValueError(
            f"stage17 autotune failed to find a valid config for shape={(batch, heads, seq_len, head_dim)} "
...
def _stage17_forward_impl(q, k, v, config: AttentionConfig):
    ...
    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE17_CUTE:
        raise ValueError(
            f"stage17 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE17_CUTE}, got {seq_len}."
        )
    if config.num_stages_kv not in {0, 2, 3, 4, 5}:
        raise ValueError(f"stage17 currently supports num_stages_kv in {{2, 3, 4, 5}}, got {config.num_stages_kv}.")
    if not Stage17FlashAttentionAmpere.can_implement(
        ...
        raise ValueError("stage17 config is not supported by the MMA kernel constraints.")
...
def stage17_forward(q, k, v, config: AttentionConfig | None = None):
    ...
    return _stage17_forward_impl(q, k, v, replace(tuned, autotune=False))


def _stage17_compile(cache_key, q_cute, k_cute, v_cute, o_cute, scale, current_stream, head_dim, block_m, block_n, num_threads, num_stages_kv):
    compiled = _STAGE17_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        kernel = Stage17FlashAttentionAmpere(
            head_dim=head_dim,
        ...
        _STAGE17_COMPILED_CACHE[cache_key] = compiled
    return compiled

...
_STAGE14_COMPILED_CACHE = {}


if HAS_CUTE:
    class Stage14FlashAttentionAmpere:
        ...
        def __call__(
            ...
            gmem_tiled_copy_Q = cute.make_tiled_copy_tv(atom_async_copy, tQKV_layout, vQKV_layout)
            gmem_tiled_copy_KV = cute.make_tiled_copy_tv(atom_async_copy, tKV_layout, vQKV_layout)
...
def _stage14_forward_impl(q, k, v, config: AttentionConfig):
    require_torch()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage14 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage14 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage14 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE14_CUTE:
        raise ValueError(
            f"stage14 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE14_CUTE}, got {seq_len}."
        )
    if not Stage14FlashAttentionAmpere.can_implement(cutlass.Float16, head_dim, config.block_m, config.block_n, config.num_threads, True):
        raise ValueError("stage14 config is not supported by the MMA kernel constraints.")
...
def stage14_forward(q, k, v, config: AttentionConfig | None = None):
    config = config or AttentionConfig()
    return _stage14_forward_impl(q, k, v, replace(config, autotune=False))


def _stage14_compile(cache_key, q_cute, k_cute, v_cute, o_cute, scale, current_stream, head_dim, block_m, block_n, num_threads):
    compiled = _STAGE14_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        kernel = Stage14FlashAttentionAmpere(
            head_dim=head_dim,
        ...
        _STAGE14_COMPILED_CACHE[cache_key] = compiled
    return compiled

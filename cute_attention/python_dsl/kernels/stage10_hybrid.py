...
MAX_SEQ_LEN_FOR_STAGE10_CUTE = 4096
_STAGE10_COMPILED_CACHE = {}


if HAS_CUTE:
    def _make_stage10_host(seq_len: int, head_dim: int, block_m: int, block_n: int, num_threads: int):
        threads_per_row = num_threads // block_m

        @cute.kernel
        def stage10_hybrid_kernel(
            q: cute.Tensor,
            k: cute.Tensor,
            v: cute.Tensor,
            o: cute.Tensor,
            ...
            for n_start in range(0, seq_len, block_n):
                ...
                if row < block_m and lane == 0:
                    ...
                    if q_idx < seq_len and n_start <= q_idx:
                        ...
                        row_m[row] = m_new
                        row_l[row] = row_l[row] * alpha[row] + block_sum
                    else:
                        alpha[row] = 1.0
                cute.arch.barrier()
        ...
        return stage10_forward_host


def stage10_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage10 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage10 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage10 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE10_CUTE:
        raise ValueError(
            f"stage10 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE10_CUTE}, got {seq_len}."
        )
    if head_dim > 256:
        raise ValueError(f"stage10 currently supports head_dim <= 256, got {head_dim}.")
    if config.block_m <= 0:
    ...
    if config.num_threads % config.block_m != 0:
        raise ValueError("stage10 requires num_threads % block_m == 0.")

    threads_per_row = config.num_threads // config.block_m
    if threads_per_row < 1:
        raise ValueError("stage10 requires threads_per_row >= 1.")

    smem_bytes = (
        2 * (config.block_m * head_dim + 2 * config.block_n * head_dim)
        + 4 * (config.block_m * config.block_n + config.block_m * head_dim + 3 * config.block_m)
    )
    if smem_bytes > 96 * 1024:
        raise ValueError(
            f"stage10 shared memory footprint too large ({smem_bytes} bytes). "
...
def _stage10_compile(cache_key, q_cute, k_cute, v_cute, o_cute, softmax_scale, block_m, block_n, num_threads):
    compiled = _STAGE10_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        seq_len = cache_key[0][1]
        head_dim = cache_key[0][2]
        stage10_host = _make_stage10_host(
            seq_len=seq_len,
            head_dim=head_dim,
            block_m=block_m,
            block_n=block_n,
        ...
        _STAGE10_COMPILED_CACHE[cache_key] = compiled
    return compiled

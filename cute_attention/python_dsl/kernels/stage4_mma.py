...
MAX_SEQ_LEN_FOR_STAGE4_CUTE = 4096
_STAGE4_COMPILED_CACHE = {}


if HAS_CUTE:
    def _make_stage4_host(seq_len: int, head_dim: int, block_m: int, block_n: int, num_threads: int):
        @cute.kernel
        def stage4_k_cache_kernel(
            q: cute.Tensor,
            k: cute.Tensor,
            v: cute.Tensor,
            o: cute.Tensor,
        ...
        return stage4_forward_host


def stage4_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage4 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage4 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage4 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE4_CUTE:
        raise ValueError(
            f"stage4 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE4_CUTE}, got {seq_len}."
        )
    if head_dim > 256:
        raise ValueError(f"stage4 currently supports head_dim <= 256, got {head_dim}.")
    if config.block_m <= 0:
    ...
    smem_bytes = (
        ...
        + 2 * (config.block_n * head_dim)
    )
    if smem_bytes > 96 * 1024:
        raise ValueError(
            f"stage4 shared memory footprint too large ({smem_bytes} bytes). "
...
def _stage4_compile(cache_key, q_cute, k_cute, v_cute, o_cute, softmax_scale, block_m, block_n, num_threads):
    compiled = _STAGE4_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        seq_len = cache_key[0][1]
        head_dim = cache_key[0][2]
        stage4_host = _make_stage4_host(
            seq_len=seq_len,
            head_dim=head_dim,
            block_m=block_m,
            block_n=block_n,
        ...
        _STAGE4_COMPILED_CACHE[cache_key] = compiled
    return compiled

...
MAX_SEQ_LEN_FOR_STAGE3_CUTE = 4096
_STAGE3_COMPILED_CACHE = {}


if HAS_CUTE:
    def _make_stage3_host(seq_len: int, head_dim: int, block_n: int, num_threads: int):
        @cute.kernel
        def blocked_online_causal_attention_kernel(
            q: cute.Tensor,
            k: cute.Tensor,
            v: cute.Tensor,
            o: cute.Tensor,
        ...
        def stage3_forward_host(
            ...
            blocked_online_causal_attention_kernel(
                q,
                k,
                v,
                o,
        ...
        return stage3_forward_host


def stage3_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage3 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage3 requires cutlass.cute.")
    if q.dtype not in [torch.float16, torch.bfloat16]:
        raise ValueError(f"stage3 currently only supports fp16/bf16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE3_CUTE:
        raise ValueError(
            f"stage3 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE3_CUTE}, got {seq_len}."
        )
    if head_dim > 256:
        raise ValueError(f"stage3 currently supports head_dim <= 256, got {head_dim}.")
    if config.block_n <= 0:
...
def _stage3_compile(cache_key, q_cute, k_cute, v_cute, o_cute, softmax_scale, block_n, num_threads):
    compiled = _STAGE3_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        seq_len = cache_key[0][1]
        head_dim = cache_key[0][2]
        stage3_host = _make_stage3_host(
            seq_len=seq_len,
            head_dim=head_dim,
            block_n=block_n,
        ...
        _STAGE3_COMPILED_CACHE[cache_key] = compiled
    return compiled

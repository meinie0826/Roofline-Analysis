...
MAX_SEQ_LEN_FOR_STAGE5_CUTE = 4096
_STAGE5_COMPILED_CACHE = {}


if HAS_CUTE:
    def _make_stage5_host(seq_len: int, head_dim: int, block_m: int, block_n: int, num_threads: int):
        @cute.kernel
        def stage5_kv_cache_kernel(
            q: cute.Tensor,
            k: cute.Tensor,
            v: cute.Tensor,
            o: cute.Tensor,
                ...
                for lin in range(tidx, block_m * block_n, num_threads):
                    ...
                    if q_idx < seq_len and kv_idx < seq_len and kv_idx <= q_idx:
                        ...
                        for d_idx in range(head_dim):
                            dot += q_tile[q_base + d_idx] * k_tile[k_base + d_idx].to(cutlass.Float32)
        ...
        return stage5_forward_host


def stage5_forward(q, k, v, config: AttentionConfig | None = None):
    require_torch()
    config = config or AttentionConfig()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage5 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage5 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage5 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE5_CUTE:
        raise ValueError(
            f"stage5 currently supports seq_len <= {MAX_SEQ_LEN_FOR_STAGE5_CUTE}, got {seq_len}."
        )
    if head_dim > 256:
        raise ValueError(f"stage5 currently supports head_dim <= 256, got {head_dim}.")
    if config.block_m <= 0:
    ...
    smem_bytes = (
        4 * (config.block_m * head_dim * 2 + config.block_m * config.block_n + 2 * config.block_m)
        + 4 * (config.block_n * head_dim)
    )
    if smem_bytes > 96 * 1024:
        raise ValueError(
            f"stage5 shared memory footprint too large ({smem_bytes} bytes). "
...
def _stage5_compile(cache_key, q_cute, k_cute, v_cute, o_cute, softmax_scale, block_m, block_n, num_threads):
    compiled = _STAGE5_COMPILED_CACHE.get(cache_key)
    if compiled is None:
        seq_len = cache_key[0][1]
        head_dim = cache_key[0][2]
        stage5_host = _make_stage5_host(
            seq_len=seq_len,
            head_dim=head_dim,
            block_m=block_m,
            block_n=block_n,
        ...
        _STAGE5_COMPILED_CACHE[cache_key] = compiled
    return compiled

...
def _make_stage13_config(config: AttentionConfig, *, block_m: int, block_n: int, num_stages_kv: int) -> AttentionConfig:
...
def autotune_stage13_config(
    ...
    *,
    ...
    if not config.causal:
        raise ValueError("stage13 autotune only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage13 autotune requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage13 autotune currently only supports fp16 inputs, got {q.dtype}.")
    ...
    if best_config is None:
        raise ValueError(
            f"stage13 autotune failed to find a valid config for shape={(batch, heads, seq_len, head_dim)} "
...
def _stage13_forward_impl(q, k, v, config: AttentionConfig):
    require_torch()
    validate_qkv(q, k, v)
    if not config.causal:
        raise ValueError("stage13 only supports causal attention.")
    if not HAS_CUTE:
        raise RuntimeError("stage13 requires cutlass.cute.")
    if q.dtype != torch.float16:
        raise ValueError(f"stage13 currently only supports fp16 inputs, got {q.dtype}.")

    batch, heads, seq_len, head_dim = q.shape
    if seq_len > MAX_SEQ_LEN_FOR_STAGE13_CUTE:
        raise ValueError(
...
def stage13_forward(q, k, v, config: AttentionConfig | None = None):
    config = config or AttentionConfig()
    tuned = config
    if config.autotune:
...
def _stage13_compile(cache_key, q_cute, k_cute, v_cute, o_cute, scale, current_stream, head_dim, block_m, block_n, num_threads, num_stages_kv):
    ...
    if compiled is None:
        ...
        _STAGE13_COMPILED_CACHE[cache_key] = compiled
    return compiled

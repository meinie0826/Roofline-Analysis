...
constexpr std::size_t kReadyBarrierOffsetBytes =
    align_up_local(kFilledBarrierOffsetBytes + kStages * sizeof(uint64_t),
                   alignof(uint64_t));
constexpr std::size_t kStep5SmemBytes =
...
Step5Options parse_options(int argc, char** argv) {
  Step5Options options;
  ...
  if (options.m % kTileM != 0 || options.n % kTileN != 0 ||
    ...
    throw std::runtime_error(
        "step5 hopper multistage ws tma currently requires m multiple of 64, n multiple of 32, k multiple of 16");
  }
  return options;
}
...
__global__ __launch_bounds__(kThreads) void step5_hopper_multistage_ws_tma_kernel(
...
void launch_hopper_multistage_ws_tma_gemm(const CUtensorMap& a_tmap,
  ...
  step5_hopper_multistage_ws_tma_kernel<<<grid, block, kStep5SmemBytes>>>(
      a_tmap, b_tmap, d_d, options.m, options.n, options.k);
  CHECK_CUDA(cudaGetLastError());
}

template <typename LaunchFn>
TimingStats benchmark_kernel(const Step5Options& options,
                             LaunchFn&& launch_fn,
                             float* d_out,
...
void print_step5_result_line(const Step5Options& options,
                             const char* backend,
                             const TimingStats& stats) {
  std::cout << std::fixed << std::setprecision(6)
            << "RESULT benchmark=bench_step5_hopper_multistage_ws_tma"
            << " backend=" << backend
            << " m=" << options.m
            << " n=" << options.n
            << " k=" << options.k
...
void print_step5_check_line(const Step5Options& options,
                            const char* backend,
                            const CompareStats& compare,
                            bool exact_required) {
  std::cout << std::scientific << std::setprecision(6)
            << "CHECK benchmark=bench_step5_hopper_multistage_ws_tma"
            << " backend=" << backend
            << " m=" << options.m
            << " n=" << options.n
            << " k=" << options.k
...
void print_step5_summary(const Step5Options& options,
  ...
  std::cout << "SUMMARY benchmark=bench_step5_hopper_multistage_ws_tma"
            << " m=" << options.m
            << " n=" << options.n
            << " k=" << options.k
            << " baseline=cublas_fast_16bf"
            << '\n';
  std::cout << "  correctness: hopper_multistage_ws_tma_mma vs cublas_pedantic exact_pass="
  ...
  print_row("hopper_multistage_ws_tma_mma", hopper_ws_stats,
...
int main(int argc, char** argv) {
  try {
    const Step5Options options = parse_options(argc, argv);
    print_device_info();
    ...
    if (props.major < 9) {
      throw std::runtime_error(
          "step5 hopper multistage warp-specialized tma path requires SM90 or newer");
    }
    ...
    auto hopper_ws_launch = [&] {
      launch_hopper_multistage_ws_tma_gemm(a_tmap, b_tmap, device_hopper_ws_D,
                                           options);
    };
    ...
    print_step5_check_line(options, "hopper_multistage_ws_tma_mma_vs_cublas_pedantic",
                           hopper_compare, true);
    print_step5_check_line(options, "cublas_fast_16bf_vs_cublas_pedantic",
                           fast_compare, false);

    if (!hopper_compare.pass) {
      throw std::runtime_error(
          "hopper multistage ws tma mma output does not exactly match cuBLAS");
    }
    ...
    print_step5_result_line(options, "hopper_multistage_ws_tma_mma",
                            hopper_ws_stats);
    print_step5_result_line(options, "cublas_fast_16bf", cublas_fast_stats);
    print_step5_result_line(options, "cublas_pedantic", cublas_pedantic_stats);
    print_step5_summary(options, hopper_ws_stats, cublas_fast_stats,

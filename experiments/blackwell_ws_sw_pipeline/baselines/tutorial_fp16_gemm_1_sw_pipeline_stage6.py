...
def run_dense_gemm(
    ...
    print(f"  benchmark: {do_benchmark}")
    print(f"  warmup:    {warmup_iterations}")
    print(f"  iters:     {iterations}")
    print(f"  skip_ref:  {skip_ref_check}")
    print("===================================================================")
    print()
    ...
    if not skip_ref_check:
        ref = (
            torch.einsum("mk,nk->mn", a.to(torch.float32), b.to(torch.float32))
        ).cpu()
        torch.testing.assert_close(
            c.cpu(),
            ref.to(cutlass_torch.dtype(io_dtype)),
            atol=tolerance,
            rtol=1e-05,
        )
    ...
    if do_benchmark:
        ...
        print(
            "RESULT,"
            f"variant=sw_pipeline,m={m},n={n},k={k},"
            f"avg_ms={avg_ms:.9f},tflops={tflops:.9f},iterations={iterations}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_benchmark", action="store_true", help="Run CUDA event benchmark"
    )
    parser.add_argument(
        "--warmup_iterations", type=int, default=10, help="Benchmark warmup launches"
    )
    parser.add_argument(
        "--iterations", type=int, default=100, help="Benchmark measured launches"
    )
    parser.add_argument(
